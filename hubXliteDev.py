...
def cargar_datos():
    estructura_objetivo = {
        "config": {"version": "2.7", "branch": "Xlite"},
        "modulos": {
...
if st.session_state.submodulo_activo:
    mapeo = {
        ...
        "Asesoría Medicina EPS, Prepagada, Seguros y Pensión": "julianasesora.py"
    }
    ...
    if "0." in menu:
        ...
        st.write("Soluciones médicas y asesoría integral.")
        for sub in data["modulos"]["Salud"]:
            if st.button(sub, key=f"salud_{sub}", use_container_width=True):
                st.session_state.submodulo_activo = sub
                st.rerun()

st.sidebar.markdown("---")
st.sidebar.caption(f"Versión: 2.7 | Universo")