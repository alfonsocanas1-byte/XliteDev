...
def renderizar_submodulos(nombre_modulo, titulo, icono):
    ...
    if submodulos:
        ...
        for i, sub in enumerate(submodulos):
            with cols[i]:
                # Lógica específica para redireccionar a restaurante gourmet
                if sub == "Restaurante gourmet":
                    # Botón que actúa como enlace externo a la app de Sebastián
                    st.link_button(sub, "https://sebasgourmet.streamlit.app", use_container_width=True, type="secondary")
                else:
